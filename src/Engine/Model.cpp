...
static auto addBuffer(const microgltf::Model& model, const int accessorId,
                      std::unordered_map<int, GLuint>& buffers) -> void
{
...
static auto loadTexture(const microgltf::Model& model, const int& textureId, std::unordered_map<int, GLuint>& textures,
...
auto Model::Create(const microgltf::Model& model) -> Model
{
    VertexArray vao;
