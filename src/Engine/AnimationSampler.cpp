...
auto AnimationSampler::getInput(const FrameInfo& frameInfo) const -> InputResult
{
    const float animationTime = std::fmodf(frameInfo.time.count(), m_inputMax);

    const auto next = std::upper_bound(m_inputBuffer.begin(), m_inputBuffer.end(), animationTime);
    const auto prev = next == m_inputBuffer.begin() ? next : next - 1;
    ...
    const float stepRatio = (nextVal != prevVal) ? ((animationTime - prevVal) / (nextVal - prevVal)) : 0;
...
auto AnimationSampler::update(const FrameInfo& frameInfo) -> void
{
    if (frameInfo.frameCount == m_currentFrame)
        return;

    const auto result = getInput(frameInfo);
